...
<div class="container mt-4">
	...
	<div class="row">
		<div class="col-md-5 mt-4">
			<h4 class="text-primary">Search CDS By Month</h4>
			<form action="<?php echo $_SERVER['PHP_SELF'] ?>" method = "POST">
				...
				<div class="form-group">
					<button type="submit" class="btn btn-primary btn-block" name="search_cds">
		...
		<div class="col-md-7 mt-4">
			...
			<table class="table table-dark table-hover table-stripped">
				...
				<?php
					if(isset($_POST['search_cds'])){
						require '../incs/database.inc.php';
						$month = $_POST['month'];
						$year = date("Y");
						$sql = $conn->query("SELECT * FROM `cds` WHERE `batch` = '$group' AND `month` = '$month' AND `year` = '$year'");
						// echo $sql->num_rows;
						if($sql->num_rows == 0){
							// array_push($err, "CDS Result for $month is not available");
							echo "
								<div class = 'alert alert-danger text-center'>
									<b>CDS for $month is not yet Available</b>
								</div>
							";
						}
						else{
